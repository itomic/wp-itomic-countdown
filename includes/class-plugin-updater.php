...
class Itomic_Countdown_Updater {
	...
	public function check_for_update( $transient ) {
		...
		if ( $remote_version && version_compare( $this->version, $remote_version->version, '<' ) ) {
			...
			// Format: 'plugin-dir/plugin-file.php' (e.g., 'itomic-countdown/itomic-countdown.php')
			// WordPress uses this to identify it as the SAME plugin (not a new installation)
			$plugin_data = array(
				'slug'        => dirname( $this->plugin_slug ),  // Plugin directory name
				'plugin'      => $this->plugin_slug,              // CRITICAL: Full plugin path
	...
	private function get_remote_version() {
		...
		if ( empty( $download_url ) ) {
			error_log( 'Itomic Countdown: No ZIP file found in GitHub release. Release must include itomic-countdown.zip as an asset.' );
	...
	public function plugin_info( $result, $action, $args ) {
		...
		$result = (object) array(
			...
			'author'        => '<a href="https://www.itomic.com.au">Itomic</a>',
	...
	public function after_install( $response, $hook_extra, $result ) {
		...
		if ( is_wp_error( $result ) ) {
			error_log( 'Itomic Countdown: Update installation failed: ' . $result->get_error_message() );
			return $result;
		}
